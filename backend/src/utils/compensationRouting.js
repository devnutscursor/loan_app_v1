/**
 * Compensation Type (BPC / LPC) routing helpers — backend.
 * Mirrors frontend/src/utils/compensationRouting.js — keep in sync.
 *
 * Per the client's "Compensation Type SOP" this is NOT a product-routing rule.
 * It's a pure post-processing pass over Mortech's raw quotes that:
 *   Lender Paid  → keeps the raw price. No Section A fee is added.
 *   Borrower Paid → thins the displayed price by `lenderPaidDefaultPct` and
 *                  appends an origination-charge fee in Section A equal to
 *                  `loanAmount * borrowerPaidFeePct / 100`. That fee is a
 *                  Prepaid Finance Charge (PFC) → reduces Amount Financed →
 *                  raises APR via actuarial PV (`computeAprAfterPrepaidFinanceCharges`).
 *
 * Mortech never receives the comp percentages — they are applied entirely on
 * our side so the pricing engine sees only the real lender price.
 */

const round3 = (n) => Number(Number(n || 0).toFixed(3));
const round2 = (n) => Number(Number(n || 0).toFixed(2));

/** Fixed borrower Section A fee — must match frontend constant. */
const FIXED_BORROWER_SECTION_A_FEE_PCT = 0.75;

/**
 * Borrower points $ / % from Mortech `<quote_detail price="...">` (API: `points`).
 * See frontend/src/utils/compensationRouting.js — keep in sync.
 */
function quoteDetailPointsCostAndPct(quoteDetailPrice, loanAmount) {
...
const COMP_INITIAL = Object.freeze({
  lenderPaidDefaultPct: 1.25,
  lenderPaidDefaultAmt: 0,
...
/**
 * Parse the `compensation` block from an inbound Mortech search body. Returns
 * null when the block is absent / malformed, otherwise a normalized object.
 */
function compensationFromBody(body) {
  const raw = body && body.compensation ? body.compensation : null;
  if (!raw) return null;
  const lenderPaidDefaultPct =
    toFiniteNumber(raw.lenderPaidDefaultPct) != null
      ? round3(toFiniteNumber(raw.lenderPaidDefaultPct))
...
function validateCompensation(comp) {
  const errors = {};
  const c = comp || {};
...
function buildBpcSectionAFee(loanAmount, feePct) {
...
/**
 * Per-fee decision for whether a Mortech fee line counts toward ΣPFC (APR).
 * Used for logging / audits — keep in sync with `isMortechFeePfcForApr`.
 */
...
/**
 * Audit of Mortech `fees[]` for BPC APR ΣPFC — included vs excluded with reasons.
 */
...
/**
 * Apply compensation math to a single Mortech quote object. Mutates nothing;
 * returns a new rate object with the comp fields added and the Section A PFC
 * appended to `fees[]` when BPC is active.
 *
 * Input `rate` is one element of the `transformedRates` list in the Mortech
 * controller (see shape in `transformedRatesRaw`). It must have at minimum:
 *   - apr
 *   - points (`quote_detail@price`) for display price = 100 − price; optional
 *     ratesheetPrice fallback when points is missing
 *   - fees[] (may be empty)
 */
function applyCompensationToRate({
  rate,
  loanAmount,
  compPayer,
  compensation,
  ...
  // NOTE: we intentionally do NOT push `sectionAFee` into `src.fees` here.
  // The frontend's `buildRateGroups` owns the display fees array so the LO can
  // tweak comp % in the Broker Compensation modal and see the Section A row
  // update instantly without a network round-trip. `sectionAFee` is exposed on
  // the rate object for callers (persistence, audit) that want to inspect it.
  const out = {
    ...src,
    rawBasePrice: round3(rawBasePrice),
    rawCost,
...
module.exports = {
  ...
  auditMortechFeesForBpcApr,
  mortechFeePfcVerdict,
};
