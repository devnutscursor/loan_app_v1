...
exports.createLoan = async (req, res, next) => {
  try {
    ...
    // We no longer need to generate a temporary loan number here
    // The pre-save hook in the loan model will handle this
    ...
    // Note: loanNumber is now automatically set by the pre-save hook in the model
...
exports.saveDraft = async (req, res, next) => {
  try {
    ...
    // For drafts, we use DRAFT prefix instead of a loan number
    // The actual loan number will be generated by the pre-save hook when the loan is finalized
    
    // Create minimal data to satisfy required fields but with isDraft flag
    // We're using insertMany to bypass mongoose validation
    const draftDocument = {
      borrower: borrower._id,
      // Don't set loanNumber for drafts - it will be generated automatically when finalized
      status: "draft",
...
exports.createLoanData = async (req, res, next) => {
  try {
    ...
    // The pre-save hook will automatically generate the loan number in the format YYYYMMDD001
    await newLoan.save();
