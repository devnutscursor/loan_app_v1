 ...
 */
const BorrowerStep = ({ formData, handleChange, validateStep, nextStep, prevStep, errors = {} }) => {
  ...
  const getTabIcon = (tabName) => {
    ...
    if (tabName === 'personalDetails') {
      ...
      isComplete = borrower.currentAddress && 
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center space-x-2 border-b border-gray-200 pb-3">
        <button
          ...
          style={activeTab === 'personalDetails' ? { backgroundColor: theme.colors.primary } : {}}
          onClick={() => setActiveTab('personalDetails')}
        >
          <div className="flex items-center">
            <span>Personal Details</span>
            {getTabIcon('personalDetails') && <span className="ml-2">{getTabIcon('personalDetails')}</span>}
          </div>
        </button>
        ...
        <button
          ...
          style={activeTab === 'residenceHistory' ? { backgroundColor: theme.colors.primary } : {}}
          onClick={() => setActiveTab('residenceHistory')}
        >
          <div className="flex items-center">
            <span>Residence History</span>
            {getTabIcon('residenceHistory') && <span className="ml-2">{getTabIcon('residenceHistory')}</span>}
          </div>
        </button>
        ...
        <button
          ...
          style={activeTab === 'employmentHistory' ? { backgroundColor: theme.colors.primary } : {}}
          onClick={() => setActiveTab('employmentHistory')}
        >
          <div className="flex items-center">
            <span>Employment History</span>
            {getTabIcon('employmentHistory') && <span className="ml-2">{getTabIcon('employmentHistory')}</span>}
          </div>
        </button>
      </div>
        ...
        {activeTab !== 'employmentHistory' ? (
          ...
          <button
            ...
            onClick={() => {
              ...
              if (borrower.employers && 
                  borrower.employers.length > 0 && 
                  borrower.employers[0].companyName && 
                  borrower.employers[0].jobTitle) {
                nextStep();
              } else {
                alert('Please complete all required employment information before proceeding');
              }
            }}
          >
            Next Step
