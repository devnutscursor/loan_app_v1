...
const LoanQualificationCard = ({ loan, onUpdate, enablePolling = false }) => {
  ...
  const calculateLoanValues = (updatedLoan = null) => {
    ...
    if (hasStoredCalcs && Object.keys(currentLoan.loanCalculations).length > 0) {
      ...
      const interestRate = hasStoredParams ? parseFloat(currentLoan.loanParameters.interestRate) : 0;
      ...
      setCalculations({
        ...
        programName: loanPrograms.find(program => program._id === currentLoan?.loanParameters?.selectedProgramId)?.displayName || 'Conventional',
        interestRate
      });
    } else {
      ...
      console.log('[DEBUG] Using default calculated values for new loan:', defaultCalculations);
    ...
    <div className="bg-white rounded-lg ">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-start">
        ...
        <LoanInfoSection 
          ...
          programName={calculations.programName}
        />
      </div>
