...
const MCRReports = () => {
  ...
  const handleGenerate = async () => {
    if (!selectedYear || !selectedPeriod) {
      toast.error("Please select year and period");
      return;
    }
    ...
    try {
      ...
      setStateFilter("All");
      toast.success("MCR report generated successfully");
      loadReports();
    } catch (err) {
  ...
  const handleViewReport = async (reportId) => {
    setLoading(true);
    try {
      const res = await MCRService.getReport(reportId);
      setActiveReport(res.data);
      setActiveTab("application");
      setStateFilter("All");
    } catch (err) {
  ...
  return (
    <LenderLayout>
      <Head>
        <title>MCR Reports | LoanApp360</title>
      </Head>
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs sm:text-sm text-gray-500">Generate and manage NMLS Mortgage Call Reports</p>
            </div>
          </div>
              ...
              <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    {formatDate(activeReport.startDate)} — {formatDate(activeReport.endDate)}
