...
const MCRServiceInstance = new MCRService();
...
const AdminMCRReports = () => {
  ...
  useEffect(() => {
    (async () => {
      try {
        const res = await MCRServiceInstance.getLendersForMCR();
        setLenders(res.data || []);
      } catch {
  ...
  /* ── Load reports when lender changes ── */
  const loadReports = useCallback(async () => {
    setLoadingReports(true);
    try {
      const res = await MCRServiceInstance.getReports(selectedLenderId || undefined);
      setReports(res.data || []);
    } catch { /* silent */ }
  ...
  const handleGenerate = async () => {
    if (isPeriodFuture(selectedYear, selectedPeriod)) {
      toast.error(`${selectedPeriod} ${selectedYear} has not started yet`); return;
    }
    setGenerating(true);
    try {
      const res = await MCRServiceInstance.generateReport(
        selectedYear, selectedPeriod,
        selectedStates.length > 0 ? selectedStates : undefined,
        undefined, null, selectedLenderId || undefined
      );
  ...
  const handleViewReport = async (reportId) => {
    ...
    try {
      const res = await MCRServiceInstance.getReport(reportId);
      setActiveReport(res.data);
      setActiveTab("application");
      setStateFilter("All");
  ...
  const handleDeleteReport = async () => {
    ...
    setDeleting(true);
    try {
      await MCRServiceInstance.deleteReport(reportId);
  ...
  const handleStatusUpdate = async (newStatus) => {
    ...
    try {
      await MCRServiceInstance.updateReportStatus(activeReport._id, newStatus);
  ...
  const handleExport = async (format) => {
    setExporting(true);
    try {
      const stateParam = stateFilter === "All" ? "all" : stateFilter;
      await MCRServiceInstance.exportReport(activeReport._id, format, stateParam);
  ...
  /* ── Derived ── */
  const selectedLender = lenders.find((l) => l._id === selectedLenderId);
  const availableStates = activeReport?.states || [];
  const loLabel = selectedLender ? selectedLender.name : "All Loans (Platform-wide)";
  ...
  return (
    <ProtectedRoute allowedRoles={["admin"]}>
      <MainLayout title="Admin MCR Reports">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 py-6 space-y-6">
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-2xl font-bold text-gray-900">MCR Reports</h1>
                <p className="text-sm text-gray-500">Generate &amp; review NMLS Mortgage Call Reports for any Loan Officer</p>
              </div>
            </div>
            {activeReport && (
              <button
          ...
          {/* ── LO selector (optional combobox) ── */}
          {!activeReport && (
            <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
              <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-2">
                <Users className="h-4 w-4 text-blue-600" />
                <h2 className="text-base font-semibold text-gray-900">Loan Officer</h2>
                <span className="text-xs text-gray-400 ml-1">(optional — leave blank to include all loans)</span>
              </div>
                ...
                {loadingLenders ? (
                  ...
                  <div className="flex items-center gap-3 max-w-xl">
                    <select
                      ...
                      onChange={(e) => { setSelectedLenderId(e.target.value); setActiveReport(null); }}
                    ...
                    >
                      <option value="">— All Loan Officers (platform-wide) —</option>
                      {lenders.map((l) => (
                        <option key={l._id} value={l._id}>
                          {l.name}{l.nmls ? ` (NMLS# ${l.nmls})` : ""}{l.companyName ? ` — ${l.companyName}` : ""}
                        </option>
                      ))}
                    </select>
                    {selectedLenderId && (
                      <button
                        onClick={() => { setSelectedLenderId(""); setActiveReport(null); }}
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-2">
                  <h2 className="text-base font-semibold text-gray-900">
                    Generate Report
                    <span className="text-gray-400 font-normal"> for {loLabel}</span>
                  </h2>
                </div>
                <div className="px-6 py-5">
              ...
              <div className="bg-white rounded-2xl shadow-sm border border-gray-200 overflow-hidden">
                <div className="px-6 py-4 border-b border-gray-100 flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <h2 className="text-base font-semibold text-gray-900">
                      Reports
                      <span className="text-gray-400 font-normal"> — {loLabel}</span>
                    </h2>
                  </div>
                ...
                {loadingReports ? (
                  ...
                  <div className="px-6 py-12 text-center">
                    ...
                    <p className="text-sm text-gray-400">No reports yet. Use step 2 to generate one.</p>
                  </div>
                ) : (
                  ...
                  <div className="flex items-center gap-2 flex-wrap justify-end">
                    {activeReport.status === "draft" && (
                      <button
                        ...
                        title="Mark report as final — no further edits needed"
                      >
                    ...
                    {activeReport.status === "final" && (
                      <button
                        ...
                        title="Mark as submitted — filed with NMLS"
                      >
          ...
          {!activeReport && reports.length === 0 && !loadingReports && !generating && false && (
            <div />
          )}
        </div>
